...
const dessertImages = {
  ...
  'Chocolate Mousse': 'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=800&q=80',
...
const generateMenu = () => {
  ...
  // 30 Pizzas
  const pizzaNames = Object.keys(pizzaImages);
  ...
  // 30 Sides
  const sideNames = Object.keys(sideImages);
  ...
  // 25 Beverages
  const bevNames = Object.keys(bevImages);
  bevNames.forEach((name) => {
    items.push({
      name: name,
  ...
  // 20 Desserts
  const dessertNames = Object.keys(dessertImages);
  dessertNames.forEach((name) => {
    items.push({
      name: name,
...
const runSeed = async () => {
  try {
    ...
    console.log(`Inserting ${items.length} menu items with specific images...`);
    
    await Pizza.insertMany(items);
    
    console.log('Successfully seeded 105 unique menu items with matching images!');
    process.exit(0);
  } catch (error) {
